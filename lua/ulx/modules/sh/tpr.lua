...
if SERVER then
	...
	hook.Add("PlayerSay","TPRequest",function(ply,txt)
	
		if txt:match("^[!/]tpaccept$") and ply.TPPending then
	
			dotpr(ply.TPPending,ply)
			ULib.tsayError( ply.TPPending, ply:Nick() .. " has accepted your teleport request. Teleporting...", true )
	
			return ""
		elseif txt:match("^[!/]tpdeny$") and ply.TPPending then
...
function ulx.tpr(ply,target)
	if SERVER then
		...
		ULib.tsayError( ply, "TP Request sent to " .. target:Nick(), true )
		ULib.tsayError( target, ply:Nick() .. " would like to teleport to you; you can type !tpaccept or !tpdeny to accept or deny. Expires in " .. expire .." seconds", true )
		timer.Simple(expire,function()
			if ply and target then
				if target.TPPending == ply then
...
local tpr = ulx.command(CATEGORY_NAME,"ulx tpa",ulx.tpr,"!tpa")
tpr:addParam{ type=ULib.cmds.PlayerArg }
tpr:defaultAccess(ULib.ACCESS_ALL)
tpr:help("Send a teleport request to the target player")